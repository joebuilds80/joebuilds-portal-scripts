...
const JoeBuildsDigitalTwin = (() => {
  ...
  const renderDashboard = (data) => {
    ...
    data.rooms.forEach(room => {
      ...
      // PS-053 (20 Aug 2026): the old default was 'stable', so any room with no readings at
      // all rendered as Stable — including rooms recorded on the day as No Access. A room we
      // did not measure carries no status, and a room we could not enter says so.
      const accessStatus = (room.access_status || '').toLowerCase().replace(/\s/g, '');
      let rawOverall;
      if (accessStatus === 'noaccess') rawOverall = 'noaccess';
      else if (!roomMs.length) rawOverall = 'notassessed';
      else if (statuses.includes('review') || statuses.includes('unknown') || statuses.includes('action required')) rawOverall = 'review';
    ...
    if (DOM.zonesListContainer) { 
      ...
      zoneKeys.forEach(key => { 
        ...
        btn.innerHTML = `<span class="jb-zone-label">${zone.title}</span><span class="jb-status-badge jb-status-${zone.status}">${zone.status === 'pending' ? '' : `<span class="jb-badge-dot bg-${zone.status}"></span>`}${zone.badge}</span>`; 
    ...
    if (DOM.dynamicHotspots) { 
      ...
      zoneKeys.forEach(key => { 
        ...
        const hotspotHTML = `<button class="jb-hotspot" style="left:${coords.left}; top:${coords.top};" data-id="${key}"><span class="jb-hotspot-pulse">${zone.status === 'pending' ? '' : `<span class="jb-hotspot-ping-wave bg-${zone.status}"></span>`}<span class="jb-hotspot-core-ring" style="border-color: var(--status-${zone.status});"></span></span><span class="jb-hotspot-tag">${zone.title}</span></button>`; 
