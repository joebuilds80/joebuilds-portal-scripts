 ...
 * Dashboard Controller (v26 - Remove Integrity Index & Force Title Update)
 */
const JoeBuildsDashboard = (() => {
  ...
  const getBadgeText = (status) => { 
      if (status === 'stable' || status === 'measured') return 'Stable';
      if (status === 'monitor') return 'Monitor';
      if (status === 'risk') return 'At Risk';
      if (status === 'review' || status === 'unknown' || !status) return 'Further Investigation Required';
      return 'Stable';
  };

  const getStatusVisual = (status) => {
      if (status === 'stable' || status === 'measured') return 'stable';
      if (status === 'monitor') return 'measured'; 
      if (status === 'risk') return 'risk';
      if (status === 'review' || status === 'unknown' || !status) return 'review';
      return 'stable';
  };
  ...
  const mapReadiness = (scenarios) => {
    ...
    const dotEl = card.querySelector('.jb-badge-dot');
    if (badgeEl && dotEl) {
        badgeEl.className = `jb-status-badge jb-status-stable`; 
  ...
  const init = async () => {
    ...
    // 1. Force hardcoded HTML card title update immediately
    if (DOM.metricCards) {
        DOM.metricCards.forEach(card => {
