/* ============================================================================
   ...
   V2.4 (2026-09-25, Mac): look pass towards the approved HTML version (Staging
   S1). Presentation only; every figure and word comes from the same data paths.
     Navigation   Compare and History sit in the main list, Record depth and
                  Context under "Record detail"; all four open the matching
                  Records tab. On a phone a thumb bar sits at the bottom.
     Dashboard    A hero card (the matched model inside it where the property has
                  one) with the next steps, a "Current record" panel, the next
                  useful action and record depth beside it, then the latest
                  recorded readings. Serif display headings, soft cards.
     Axe V2.3 sweep, correction 1: the room and location panel closes the same
                  way from the close button or Escape, and focus returns to what
                  opened it (the plan room, or the 3D model frame).
     Axe V2.3 sweep, correction 2: the kept property choice is removed when
                  Sign out is pressed, and is stored with the signed-in member's
                  id, so it is discarded for any other member or session.
   ========================================================================== */
/* ============================================================================
...
(function () {
  ...
  /* V2.4 (Axe V2.3 correction 2): the property choice kept for this tab is
     removed on sign out, and is tied to the signed-in member, so it is never
     reused by a different member or after the session ends another way. It is
     only ever a preference: a code is used only when that property is visible
     to the member now signed in. */
  let HS_WHO = null;
  ...
  function writeChoice(code, who) { sessionStorage.setItem('hs_property', JSON.stringify({ code: String(code), member: who || null })); }
  document.addEventListener('click', e => {
    const t = e.target;
    if (t && t.closest && t.closest('.jb-signout, #jbLogoutBtn, [data-ms-action="logout"]')) { try { sessionStorage.removeItem('hs_property'); } catch (err) {} }
  }, true);
  ...
  const NAV_ITEMS = [
    ...
    ['/diagnostics#compare', 'Compare', 'records', 'compare'],
    ['/diagnostics#history', 'History', 'records', 'history'],
    ['/diagnostics#readings', 'Records', 'records', 'readings'],
    ['/pathway', 'Pathway', 'pathway'],
    ['/reports', 'Reports', 'reports']
  ];
  const NAV_DETAIL = [
    ['/diagnostics#depth', 'Record depth', 'records', 'depth'],
    ['/diagnostics#context', 'Context', 'records', 'context']
  ];
  const TABBAR = [
    ['/dashboard', 'Home', 'dashboard'], ['/digital-twin', 'Map', 'home-map'], ['/diagnostics#compare', 'Compare', 'records', 'compare'],
    ['/diagnostics#history', 'History', 'records', 'history'], ['/diagnostics#readings', 'Records', 'records', 'readings'], ['/reports', 'Reports', 'reports']
  ];

  const MAIN_BODY = {
    dashboard:
      '<div class="hs-dash-top"><header id="hsHero" class="hs-hero-card"></header><aside class="hs-dash-side" id="hsDashSide"></aside></div>' +
      '<h2 class="hs-sec-title"><small>Latest recorded readings</small>What was recorded at the last visit</h2>' +
  ...
  function buildShell(screen) {
    ...
    const link = function (n) {
      return '<a href="' + n[0] + '" data-hs-screen-link="' + n[2] + '"' + (n[3] ? ' data-hs-tab-link="' + n[3] + '"' : '') + (n[2] === screen && !n[3] ? ' class="hs-active"' : '') + '>' + n[1] + '</a>';
    };
    const nav = NAV_ITEMS.map(link).join('') + '<div class="hs-nav-label">Record detail</div>' + NAV_DETAIL.map(link).join('');
    const tabbar = '<nav class="hs-tabbar" aria-label="Home State sections">' + TABBAR.map(link).join('') + '</nav>';
  ...
  async function loadAll() {
    ...
    try { if (chosen) writeChoice(chosen.building_code, who); else sessionStorage.removeItem('hs_property'); } catch (e) {}
  ...
  function renderHero(d) {
    ...
    if ((d.buildings || []).length > 1) {
      ...
      pick.querySelector('select').addEventListener('change', e => {
        try { writeChoice(e.target.value, HS_WHO); } catch (err) {}
        const u = new URL(location.href); u.searchParams.set('p', e.target.value); location.href = u.pathname + u.search + u.hash;
      });
    }
  }

  /* V2.4 dashboard hero: the record's identity, what it is, and the next steps.
     The matched model sits inside the card; an unmatched property shows none. */
  function dashHero(d, h) {
    const b = d.building, demo = b.record_class === 'demo' || String(b.building_code || '').startsWith('DEMO-');
    const vs = (d.sessions || []).filter(s => s.assessment_date && d.meas.some(m => m.assessment_id === s.assessment_id));
    const title = h.querySelector('.hs-hero-title');
    const eyebrow = h.querySelector('.hs-hero-eyebrow');
    if (demo && eyebrow) eyebrow.textContent = 'Demonstration property';
    const desc = el('p', 'hs-hero-desc', demo
      ? 'An illustrative record of what a measured home record holds: the home as recorded at each visit, and the same locations read again at later visits. Synthetic values, not a client home.'
      : 'Your measured home record: what was recorded at each visit, where it was recorded, and how.');
    const chips = el('div', 'hs-chips', [`${d.rooms.filter(r => !isNoAccess(r)).length} recorded spaces`, `${d.points.length} mapped locations`, `${vs.length} ${vs.length === 1 ? 'visit' : 'visits'}`, demo ? 'Synthetic record' : ''].filter(Boolean).map(t => `<span>${esc(t)}</span>`).join(''));
    const meta = h.querySelector('.hs-hero-meta');
    (meta || h.lastChild).insertAdjacentElement('beforebegin', desc);
    desc.insertAdjacentElement('afterend', chips);
    const mm = matchedModel(d);
    if (mm) {
      const f = document.createElement('iframe');
      f.className = 'hs-hero-model'; f.src = mm.url; f.title = 'Three dimensional model of this demonstration property';
      f.setAttribute('loading', 'lazy'); f.setAttribute('referrerpolicy', 'no-referrer'); f.setAttribute('sandbox', 'allow-scripts');
      h.appendChild(f);
    } else if (h.parentElement) h.parentElement.classList.add('hs-dash-top-flat');
    const act = el('div', 'hs-hero-actions', `<a class="hs-btn" href="/digital-twin">Explore the home</a>` +
      (vs.length >= 2 ? `<a class="hs-btn hs-btn-light" href="/diagnostics#compare">Compare visits</a>` : '') + `<a class="hs-btn hs-btn-light" href="/diagnostics#history">View history</a>`);
    h.appendChild(act);
  }
  ...
  function renderDashboard(d) {
    ...
    const heroEl = $('#hsHero');
    const side = $('#hsDashSide');
    if (heroEl && side) dashHero(d, heroEl);
    if (heroEl && !$('#hsGlance')) {
      ...
      if (side) {
        const cr = el('section', 'hs-side-card'); cr.innerHTML = '<small class="hs-eyebrow">Current record</small><h3 class="hs-serif">What this record holds</h3>'; cr.appendChild(g); side.appendChild(cr);
      } else heroEl.insertAdjacentElement('afterend', g);
      if (visits.length >= 2) {
        const nx = el('article', 'hs-card hs-next'); nx.id = 'hsNextCard';
        nx.innerHTML = `<div><small class="hs-eyebrow">Next useful action</small><h3 class="hs-serif">Compare the same locations across visits</h3><p class="hs-metric-desc">Look at the same mapped locations at your first and latest visits, side by side.</p></div><a class="hs-btn" href="/diagnostics#compare">Compare visits</a>`;
        if (side) { nx.classList.add('hs-side-card', 'hs-side-green'); side.appendChild(nx); } else g.insertAdjacentElement('afterend', nx);
      }
      ...
      dp.innerHTML = `<div><small class="hs-eyebrow">Property record depth</small><h3 class="hs-serif">Layers on this record</h3><p class="hs-metric-desc">${avail} of ${ls.length} layers hold readings${lim ? `, ${lim} recorded with limited access` : ''}.</p>` +
        `<div class="hs-bar" role="img" aria-label="${avail} of ${ls.length} layers hold readings"><span style="width:${ls.length ? Math.round(avail / ls.length * 100) : 0}%"></span></div></div><a class="hs-btn hs-btn-light" href="/diagnostics#depth">See each layer</a>`;
      if (side) { dp.classList.add('hs-side-card'); side.appendChild(dp); } else ($('#hsNextCard') || g).insertAdjacentElement('afterend', dp);
    }
  ...
  function openModelSelection(d, id) {
    ...
    if (pt) {
      ...
      armPanel(panel, document.querySelector('.hs-model-frame'));
      return;
    }
    ...
    if (room) openRoomPanel(d, room, document.querySelector('.hs-model-frame'));
  }

  /* V2.4 (Axe V2.3 correction 1): one close path for the room and location
     panel. The close button and Escape both hide the panel and return focus to
     what opened it: the plan room, or the 3D model frame for a selection made
     in the model. */
  let panelOpener = null, panelKeyBound = false;
  function closePanel(panel) {
    if (!panel || panel.hidden) return;
    panel.hidden = true;
    const o = panelOpener; panelOpener = null;
    if (o && document.contains(o)) { try { o.focus({ preventScroll: true }); } catch (e) { o.focus(); } }
  }
  function armPanel(panel, opener) {
    panelOpener = opener || null;
    const btn = panel.querySelector('.hs-panel-close');
    if (btn) { btn.addEventListener('click', () => closePanel(panel)); btn.focus({ preventScroll: true }); }
    if (!panelKeyBound) {
      panelKeyBound = true;
      document.addEventListener('keydown', e => {
        if (e.key !== 'Escape' || document.querySelector('.hs-viewer')) return;
        const p = $('#hsRoomPanel'); if (p && !p.hidden) { e.preventDefault(); closePanel(p); }
      });
    }
  }
  ...
  function renderRecords(d) {
    ...
    function showTab(t) {
      ...
      document.querySelectorAll('[data-hs-tab-link]').forEach(a => a.classList.toggle('hs-active', a.dataset.hsTabLink === t));
