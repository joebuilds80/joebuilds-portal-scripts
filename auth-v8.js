 ...
 * Auth Controller (v8 - least-privilege profile insert; was v7)
 */
const JoeBuildsAuth = (() => {
  const SUPABASE_URL = 'https://jsqyfiwkbuvuajwzbjhd.supabase.co';
  const SUPABASE_ANON_KEY = 'sb_publishable_xla-awjgU2npKzrWHdYKDQ_2ox0CWmC';
  
  // The specific ID for DEMO-001. Retained for reference only — no longer sent
  // from the browser. The demo view is served by the existing `jb_demo_mode`
  // client-side fallback in pathway / reports / digital-twin.
      ...
      DOM.signUpForm.addEventListener('submit', async (e) => {
        ...
        try {
          ...
          if (member.data && member.data.id) {
            ...
            // 4. Insert the profile using ONLY the columns anon/authenticated hold
            //    INSERT privilege on. `role` and `building_id` are deliberately
            //    NOT named here: the browser must never assert its own privilege
            //    level. Both are set server-side (role defaults to 'client').
            //    Naming them here is what broke sign-up after the PS-013 fix.
            ...
            if (profileErr) {
               ...
               DOM.signUpError.textContent =
                 "Your account was created, but we could not finish setting up your home record. Please contact Joe Builds before signing in.";
               DOM.signUpError.classList.remove('jb-hidden');
               DOM.btnSignUpSubmit.disabled = false;
               DOM.btnSignUpSubmit.innerHTML = `Create Account`;
               return; // do NOT send the client onward into a half-provisioned session
            }
          }
